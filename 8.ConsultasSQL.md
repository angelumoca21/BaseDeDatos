...
## Consulta simple
Sirven para ver datos de una sola tabla, sin relaciones.
`SELECT * FROM tabla;`

Trae solo columnas específicas.
`SELECT columna1, columna2 FROM tabla;`

## Consultas con filtros (WHERE)
Permiten seleccionar solo los registros que cumplen una condición.
`SELECT * FROM tabla WHERE columna = valorEspecifico;`

## Consultas ordenadas
Sirven para ordenar el resultado.
```SELECT * FROM tabla ORDER BY columnaDeOrdenamiento DESC/ASC;```
...
El producto cartesiano de dos conjuntos, es una operación que consiste en obtener otro conjunto cuyos elementos son todas las parejas que pueden formarse entre los dos conjuntos. Por ejemplo, tendríamos que tomar el primer elemento del primer conjunto y formar una pareja con cada uno de los elementos del segundo conjunto. Una vez hecho esto, repetimos el mismo proceso para cada uno de los elementos del primer conjunto.
...
### JOIN
Se usan cuando la información está repartida en varias tablas.
